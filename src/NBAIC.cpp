...
string get_title(bool print_NBAIC, bool print_BDO, bool is_end);
	...
	while (!BFS.empty()) {
		...
		for (EVENT e = 0; e < CD.size(); ++e) {
			if (fsm->transitions[current].find(e)
				!= fsm->transitions[current].end()) {
				if ((mode == BSCOPNBMAX && fsm->controllable[e]) ||
...
void NBAIC::print(bool print_BDO /* = false */) {
	os << "*******************************************************************\n"
	   << "*************" << get_title(mode == BSCOPNBMAX, print_BDO, false)
	   << "*****************\n"
	   << "*******************************************************************\n";
	os << "Y-States:\n";
	for (YS* ys : YSL) {
		...
		for (auto& pair : ys->transition)
			os << "\t\tZ-State with Information State {"
			   ...
			   << "} via "
			   << (mode == BSCOPNBMAX ? "Control Decision" : "Sensing Decision")
			   << " {"
	...
	os << "*******************************************************************\n"
	   << "*************" << get_title(mode == BSCOPNBMAX, print_BDO, true)
	   << "*************\n"
	   << "*******************************************************************\n";
}
...
string get_title(bool print_NBAIC, bool print_BDO, bool is_end) {
	...
	if (print_NBAIC) return ending + "Non-Blocking All Inclusive Controller";
